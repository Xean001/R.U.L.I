...
public static class ConfigurarCilindro
{
    public static void Execute()
    {
        const string spritePath = "Assets/ObjetosN1/N1/cilindro.png";
        const string animPath = "Assets/ObjetosN1/N1/cilindro.anim";

        var fileIdsAnim = ExtraerFileIdsDeAnim(animPath);
        Debug.Log($"FileIDs encontrados en cilindro.anim: {fileIdsAnim.Count}");
        ...
        foreach (var id in fileIdsAnim)
        {
            ...
            {
                if (TryGetLocalId(s, out long localId) && localId == id)
                {
        ...
        Debug.Log($"Frames de destruccion a usar: {spritesEnOrden.Count}");

        var scene = EditorSceneManager.GetActiveScene();
        GameObject cilindro = null;
        foreach (var root in scene.GetRootGameObjects())
            if (root.name == "cilindro_0") { cilindro = root; break; }
        if (cilindro == null) { Debug.LogError("cilindro_0 no encontrado"); return; }

        Undo.RegisterFullObjectHierarchyUndo(cilindro, "Configurar cilindro");

        var anim = cilindro.GetComponent<Animator>();
        if (anim != null) Object.DestroyImmediate(anim);

        var box = cilindro.GetComponent<BoxCollider2D>();
        if (box != null) Object.DestroyImmediate(box);
        var circle = cilindro.GetComponent<CircleCollider2D>();
        if (circle != null) Object.DestroyImmediate(circle);
        var polyExistente = cilindro.GetComponent<PolygonCollider2D>();
        if (polyExistente != null) Object.DestroyImmediate(polyExistente);

        var poly = cilindro.AddComponent<PolygonCollider2D>();
        poly.isTrigger = false;

        var sr = cilindro.GetComponent<SpriteRenderer>();
        if (sr != null && spritesEnOrden.Count > 0) sr.sprite = spritesEnOrden[0];

        var script = cilindro.GetComponent<Cilindro>();
        if (script == null) script = cilindro.AddComponent<Cilindro>();
        ...
        EditorUtility.SetDirty(cilindro);
        EditorSceneManager.MarkSceneDirty(scene);
        EditorSceneManager.SaveScene(scene);
        Debug.Log($"Cilindro configurado con {spritesEnOrden.Count} frames.");
    }
    ...
    static bool TryGetLocalId(Object obj, out long localId)
    {
        localId = 0;
        return AssetDatabase.TryGetGUIDAndLocalFileIdentifier(obj, out _, out localId);
    }
}
